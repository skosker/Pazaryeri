...
import { bankNameFromIban } from "@/lib/turkish-banks";
...
export default async function FreelancerIbansPage(props: PageProps<"/admin/freelancer-ibanlari">) {
  ...
  const bank = toSingle(searchParams.banka).trim();
  const searched = Boolean(q || bank);

  const rows = searched
    ? await prisma.freelancerIban.findMany({
        where: q ? { name: { contains: q, mode: "insensitive" } } : {},
        orderBy: { name: "asc" },
      })
    : [];

  // The bank isn't a column — it's read off the IBAN itself (see turkish-banks.ts) — so
  // that filter is applied here rather than in the query above.
  const list = bank
    ? rows.filter((row) => bankNameFromIban(row.iban).toLowerCase().includes(bank.toLowerCase()))
    : rows;
  ...
  return (
    <div>
      ...
      <form method="get" className="mt-6 flex flex-wrap items-end gap-3">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">Banka Adı</label>
          <input
            type="text"
            name="banka"
            defaultValue={bank}
            placeholder="ör. Garanti BBVA"
            className="w-48 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-purple-400"
          />
        </div>
        <button
          type="submit"
      ...
      {!searched ? (
        <p className="mt-8 rounded-2xl border border-dashed border-slate-300 p-10 text-center text-sm text-slate-400">
          IBAN&apos;ları listelemek için ad soyad ya da banka adına göre arama yapın.
        </p>
      ) : (
        <>
              ...
              <tbody>
                {list.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-5 py-4 text-slate-500">{bankNameFromIban(row.iban)}</td>

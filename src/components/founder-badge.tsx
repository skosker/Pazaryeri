/** Kalıcı "Kurucu Freelancer" rozeti — ilk 100 gerçek freelancer'a verilir. */
export function FounderBadge({ number, compact = false }: { number: number; compact?: boolean }) {
  return (
    <span
      title={`Prosinta'nın ilk 100 freelancer'ından biri (#${number})`}
    ...
    >
      ...
      {compact ? "Kurucu" : `Kurucu Freelancer #${number}`}
    </span>
  );
}
